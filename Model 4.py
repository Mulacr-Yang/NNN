...
def simulate_network(input_neurons, hidden_neurons, output_neurons, initial_states, T=200, dt=0.01):
    ...
    initial_states_hidden = [
        ...
        [-75.0, 0.35], [-75.0, 0.45], [-72.0, 0.25], [-72.0, 0.35],
        [-78.0, 0.3], [-78.0, 0.4], [-70.0, 0.2], [-70.0, 0.3],
        [-80.0, 0.3], [-80.0, 0.4], [-70.0, 0.2], [-70.0, 0.3],
        [-75.0, 0.35], [-75.0, 0.45], [-72.0, 0.25], [-72.0, 0.35],
        [-78.0, 0.3], [-78.0, 0.4], [-70.0, 0.2], [-70.0, 0.3]
    ]
    initial_states_output = [
        [-80.0, 0.3], [-80.0, 0.4], [-75.0, 0.3], [-75.0, 0.4],
        [-78.0, 0.3], [-78.0, 0.4], [-72.0, 0.3], [-72.0, 0.4]
    ]
...
def plot_timeseries(t, V_input, V_hidden, V_output, input_neurons, save_folder, sample_idx):
    ...
    for i in range(len(V_output)):
        ax.plot(t, V_output[i], label=f'Output Neuron {i + 1}', color='blue' if i < 4 else 'red', linewidth=2)
...
def plot_phase_portrait(V_input, n_input, V_hidden, n_hidden, V_output, n_output, input_neurons, hidden_neurons,
    ...
    for i in range(len(output_neurons)):
        ...
        ax.streamplot(V_grid, n_grid, dV_dt, dn_dt, color='gray', linewidth=0.5, density=1.0)
        ax.plot(V_output[i], n_output[i], color='blue' if i < 4 else 'red', lw=2, label=f'Output Neuron {i + 1}')
...
def evaluate_output(V_output, T, dt):
    """评估输出层，基于尖峰频率分类"""
    spikes = [detect_spikes(V_output[i], dt) for i in range(8)]
    freqs = [compute_spike_frequency(spikes[i], T) for i in range(8)]
    freq_sum_1 = sum(freqs[:4])  # 前 4 个神经元
    freq_sum_2 = sum(freqs[4:])  # 后 4 个神经元
    return 1 if freq_sum_1 > freq_sum_2 else 0

def main():
    # 创建神经元
    input_neurons = [
        NeuronModel(**generate_neuron_parameters(i)) for i in range(32)
    ]
    hidden_neurons = [
        NeuronModel(**generate_neuron_parameters(i)) for i in range(32, 56)
    ]
    output_neurons = [
        NeuronModel(**generate_neuron_parameters(i)) for i in range(56, 64)
    ]
