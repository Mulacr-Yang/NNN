...
def generate_input_data(num_samples=100, T=200, dt=0.01):
    ...
    for _ in range(num_samples):
        ...
        initial_states = np.array(initial_states)  # Shape: (16, 2)
...
def evaluate_output(V_output, T, dt):
    ...
    spikes_3 = detect_spikes(V_output[2], dt)
    spikes_4 = detect_spikes(V_output[3], dt)
    freq_1 = compute_spike_frequency(spikes_1, T)
    freq_2 = compute_spike_frequency(spikes_2, T)
    freq_3 = compute_spike_frequency(spikes_3, T)
    freq_4 = compute_spike_frequency(spikes_4, T)
...
def main():
    ...
    hidden_neurons = [
        NeuronModel(**generate_neuron_parameters(i)) for i in range(16, 28)
    ]
    output_neurons = [
