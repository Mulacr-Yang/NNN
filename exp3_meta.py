...
def generate_input_currents(num_samples=10, T=200, dt=0.01):
    np.random.seed(42)
    t = np.arange(0, T, dt)
    currents = []
...
def simulate_network(hidden_neurons, output_neurons, I_ext, T=200, dt=0.01):
    ...
    initial_states_hidden = [
        ...
        [-80.0, 0.5], [-70.0, 0.4], [-80.0, 0.2], [-70.0, 0.5],
        [-80.0, 0.3], [-70.0, 0.4]
    ]
...
def main():
    ...
    currents, t = generate_input_currents(num_samples=10, T=200, dt=0.01)

    output_path = os.path.join(save_folder, 'output_endpoints.txt')
    with open(output_path, 'w', encoding='utf-8') as f:
        f.write("Sample,V1,n1,V2,n2\n")
        for idx, I_ext in enumerate(tqdm(currents, desc="Simulating samples")):
            ...
            f.write(f"{idx},{V_end[0]:.4f},{n_end[0]:.4f},{V_end[1]:.4f},{n_end[1]:.4f}\n")
    print(colored(f"Saved output endpoints to {output_path}", 'green'))
